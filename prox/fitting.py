...
sys.path.append('.')
from misc import utils as my_utils
    ...
    def __enter__(self):
        ...
        if self.visualize:
            if self.viz_mode == 'o3d':
                ...
                self.area = o3d.geometry.PointCloud()
                self.crop_scene = o3d.geometry.PointCloud()
                self.sv = o3d.geometry.PointCloud()
            else:
        ...
        def fitting_func(backward=True):
            ...
            if self.visualize:
                ...
                if self.steps == 0 and self.viz_mode == 'o3d':
                    ...
                    self.body_o3d.transform(udf_data['cam2world'])
                    self.body_o3d.compute_vertex_normals()
                    self.vis_o3d.add_geometry(self.body_o3d)
                    
                    self.scene.points = udf_data['scene_o3d'].vertices
                    self.scene.colors = udf_data['scene_o3d'].vertex_colors
                    self.vis_o3d.add_geometry(self.scene)
                    
                    self.vis_o3d.add_geometry(self.axis)
                    
                    self.vis_o3d.add_geometry(self.area)
                    self.vis_o3d.add_geometry(self.crop_scene)
                    self.vis_o3d.add_geometry(self.sv)

                    if scan_tensor is not None:
                        self.scan.points = o3d.utility.Vector3dVector(scan_tensor.detach().cpu().numpy().squeeze())
                        N = np.asarray(self.scan.points).shape[0]
                        self.scan.colors = o3d.utility.Vector3dVector(np.tile([1.00, 0.75, 0.80], [N, 1]))
                        self.scan.transform(udf_data['cam2world'])
                    ...
                    if self.viz_mode == 'o3d':
                        ...
                        if udf_data['area_points'] is not None:
                            self.area.points = o3d.utility.Vector3dVector(udf_data['area_points'].detach().cpu().numpy().squeeze())
                            N = np.asarray(self.area.points).shape[0]
                            self.area.colors = o3d.utility.Vector3dVector(np.tile([0.20, 0.75, 0.80], [N, 1]))
                            self.vis_o3d.update_geometry(self.area)

                        if udf_data['scene_points'] is not None:
                            self.crop_scene.points = o3d.utility.Vector3dVector(udf_data['scene_points'])
                            N = np.asarray(self.crop_scene.points).shape[0]
                            self.crop_scene.colors = o3d.utility.Vector3dVector(np.tile([1., 1., 0.], [N, 1]))
                            self.vis_o3d.update_geometry(self.crop_scene)

                        if ray_data['sv_points'] is not None:
                            self.sv.points = o3d.utility.Vector3dVector(ray_data['sv_points'].detach().cpu().numpy().squeeze())
                            N = np.asarray(self.sv.points).shape[0]
                            self.sv.colors = o3d.utility.Vector3dVector(np.tile([0.90, 0.20, 0.80], [N, 1]))
                            self.sv.transform(udf_data['cam2world'])
                            self.vis_o3d.update_geometry(self.sv)

                        self.vis_o3d.poll_events()
                        self.vis_o3d.update_renderer()
                    else:
                        self.mv.update_mesh(vertices, body_model.faces)
            self.steps += 1
            
            #! JOINTMAPPER
            self.body_center = body_model_output.joints.detach().cpu().numpy().squeeze()[8]
...
class SMPLifyLoss(nn.Module):
    ...
    def __init__(self, search_tree=None,
        ...
        udf_loss_weight = 0.0
        self.use_udf = kwargs.get('use_udf', False)
        udf_rho = kwargs.get('udf_rho', 1e-1)
        if self.use_udf:
            self.register_buffer('udf_loss_weight', 
                                 torch.tensor(udf_loss_weight, dtype=dtype))
            self.udf_robustifier = utils.GMoF(rho=udf_rho)
        
        #! ###########################################################
        #! RAY loss
        
        ray_dist_loss_weight = 0.0
        ray_sv2m_loss_weight = 0.0
        ray_m2sv_loss_weight = 0.0
        self.use_ray = kwargs.get('use_ray', False)
        sv2m_rho = kwargs.get('sv2m_rho', 1e-1)
        m2sv_rho = kwargs.get('m2sv_rho', 1e-1)
        if self.use_ray:
            self.register_buffer('ray_dist_loss_weight', 
                                 torch.tensor(ray_dist_loss_weight, dtype=dtype))
            self.register_buffer('ray_sv2m_loss_weight', 
                                 torch.tensor(ray_sv2m_loss_weight, dtype=dtype))
            self.sv2m_robustifier = utils.GMoF(rho=sv2m_rho)
            self.register_buffer('ray_m2sv_loss_weight', 
                                 torch.tensor(ray_m2sv_loss_weight, dtype=dtype))
            self.m2sv_robustifier = utils.GMoF(rho=m2sv_rho)
        
        #! ###########################################################
        self.use_vol = kwargs.get('use_vol', True)
    ...
    def forward(self, body_model_output, camera, gt_joints, joints_conf,
                ...
                udf_data=None, ray_data=None, other_data=None,
                **kwargs):
        ...
        if (self.s2m or self.m2s) and (self.s2m_weight > 0 or self.m2s_weight > 0) and scan_tensor is not None:
            ...
            if self.s2m and self.s2m_weight > 0 and vis.sum() > 0:
                #! scan to body distance for every depth point
            ...
            if self.m2s and self.m2s_weight > 0 and vis.sum() > 0:
                #! body to scan distance for every body vertex [only body, no head]
        ...
        #! ###########################################################
        #! RAY & UDF loss
        vertice_pairs = other_data['vertice_pairs']

        #! ###########################################################
        #! RAY loss [camera] space
        #* |cam2scan - cam2body(vis, body_mask)|
        
        ray_dist_loss = 0.0
        if self.use_ray and self.ray_dist_loss_weight > 0:
            vertices_np = body_model_output.vertices.detach().cpu().numpy().squeeze()
            body_faces_np = body_model_faces.detach().cpu().numpy().reshape(-1, 3)
            m = Mesh(v=vertices_np, f=body_faces_np)
            (vis, n_dot) = visibility_compute(v=m.v, f=m.f, cams=np.array([[0.0, 0.0, 0.0]]))
            ...
            body_vertices = body_model_output.vertices[:, np.where(np.logical_and(vis > 0, self.body_mask))[0], :]
            cam2body_dist = torch.norm(body_vertices-ray_data['cam_pos'], dim=2).mean()
            ray_dist_loss = self.ray_dist_loss_weight * (ray_data['cam2scan_dist'] - cam2body_dist)
            
            # print(ray_dist_loss.item())
        
        #! ###########################################################
        #! RAY loss [camera] space
        #* debug (vis): cd(visible body_vertices, sv_points)
        #* ablation: cd(body_vertices, sv_points)
        #* ours: cd(volume_points, sv_points)
        
        ray_sv2m_loss = 0.0
        ray_m2sv_loss = 0.0
        sv_points = ray_data['sv_points']
        
        if self.use_ray and \
            (self.ray_sv2m_loss_weight > 0 or self.ray_m2sv_loss_weight > 0) and \
                sv_points is not None:
            # vertices_np = body_model_output.vertices.detach().cpu().numpy().squeeze()
            # body_faces_np = body_model_faces.detach().cpu().numpy().reshape(-1, 3)
            # m = Mesh(v=vertices_np, f=body_faces_np)
            # (vis, n_dot) = visibility_compute(v=m.v, f=m.f, cams=np.array([[0.0, 0.0, 0.0]]))
            ...
            if self.use_vol:
                volume_points = my_utils.get_volume_points_tensor(body_model_output.vertices, vertice_pairs)
                # print(volume_points.shape)
            
            #! sv to body distance for every sv
            if self.ray_sv2m_loss_weight > 0:    
                if self.use_vol:
                    sv2m, _, _, _ = distChamfer(sv_points, volume_points)
                else:
                    # body_vertices = body_model_output.vertices[:, np.where(vis == 0)[0], :]
                    body_vertices = body_model_output.vertices.clone()
                    sv2m, _, _, _ = distChamfer(sv_points, body_vertices)
                
                sv2m = self.sv2m_robustifier(sv2m.sqrt())
                ray_sv2m_loss = self.ray_sv2m_loss_weight * sv2m.sum()
                # print('sv2m: ', ray_sv2m_loss.item())
            
            #! body to sv for every body
            if self.ray_m2sv_loss_weight > 0:
                if self.use_vol:
                    #* 3
                    _, m2sv, _, _ = distChamfer(sv_points, volume_points)
                else:
                    # body_vertices = body_model_output.vertices[:, np.where(np.logical_and(vis == 0, self.body_mask))[0], :]
                    body_vertices = body_model_output.vertices.clone()
                    _, m2sv, _, _ = distChamfer(sv_points, body_vertices)
                
                m2sv = self.m2sv_robustifier(m2sv.sqrt())
                ray_m2sv_loss = self.ray_m2sv_loss_weight * m2sv.sum()
                # print('m2sv: ', ray_m2sv_loss.item())
        ...
        #! ###########################################################
        #! UDF loss [world] space
        #* ablation: pm(body_vertices, area_points)
        #* ours: pm(volume_points, area_points)
        
        udf_loss = 0.0
        area_points = udf_data['area_points']
        if self.use_udf and self.udf_loss_weight > 0 and area_points is not None:
            #! body to area points for every body
            if self.use_vol:
                volume_points = my_utils.get_volume_points_tensor(vertices, vertice_pairs)
                _, body2area, _, _ = distChamfer(area_points, volume_points)
            else:
                _, body2area, _, _ = distChamfer(area_points, vertices)
            
            body2area = self.udf_robustifier(body2area.sqrt())
            udf_loss = self.udf_loss_weight * body2area.sum()
            # print(udf_loss.item())
        ...
        if self.sdf_penetration and self.sdf_penetration_weight > 0:
            ...
            # print('sdf: ', sdf_penetration_loss.item())
        ...
        #! DON'T FORGET TO ADD THE LOSS TO TOTAL LOSS
