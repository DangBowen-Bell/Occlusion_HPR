...
class Projection():
    ...
    def create_scan(self, mask, depth_im, color_im=None, mask_on_color=False, coord='color', TH=1e-2, default_color=[1.00, 0.75, 0.80]):
        ...
        #* Filter noise points in scan (same as utils)  
        tmp_points = points[ind]
        tmp_colors = colors[ind]
        if len(tmp_points) == 0:
            return {'points':tmp_points, 'colors':tmp_colors}   
        
        tmp_center = np.mean(tmp_points, axis=0)
        cropped_points_idxs = np.where((tmp_points[:, 0] - tmp_center[0])**2 +
                                       (tmp_points[:, 1] - tmp_center[1])**2 +
                                       (tmp_points[:, 2] - tmp_center[2])**2 < 1.0**2)
        final_points = tmp_points[cropped_points_idxs]
        final_colors = tmp_colors[cropped_points_idxs]
        
        return {'points':final_points, 'colors':final_colors}   
