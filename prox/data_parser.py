...
class OpenPose(Dataset):
    ...
    def __init__(self, data_folder, img_folder='images',
        ...
        recording_name = osp.basename(data_folder)
        if 'vicon' in recording_name:
            self.img_paths = sorted([osp.join(self.img_folder, img_fn)
            ...
            self.img_paths = sorted([osp.join(self.img_folder, frame_id + '.jpg') 
                                     for frame_id in constants.prox_frames_test[recording_name]])
    
        print(recording_name, ': ', len(self.img_paths))
